...
import { userService, type UserSubscriptionPlan } from "../../../services/userService";
...
const DURATION_LABELS = { monthly: 'ماهانه', quarterly: 'سه‌ماهه', yearly: 'سالانه' };
...
export default function UserSubscriptions() {
  ...
  const getPlanStatus = (planId: 'bronze' | 'silver' | 'gold') => {
    if (!currentSubscription) {
      return { status: 'available', buttonText: `خرید پلن ${getPlanName(planId)}`, description: '' };
    }
    ...
    if (targetPlanOrder === currentPlanOrder && targetDurationOrder === currentDurationOrder) {
      return { status: 'active', buttonText: 'فعال', description: 'اشتراک فعلی شما' };
    }

    if (targetPlanOrder < currentPlanOrder) {
      return { status: 'unavailable', buttonText: 'در دسترس نیست', description: 'پلن پایین‌تر از اشتراک فعلی شماست' };
    }
    ...
    if (targetPlanOrder > currentPlanOrder || (targetPlanOrder === currentPlanOrder && targetDurationOrder > currentDurationOrder)) {
      ...
      return { status: 'upgrade', buttonText: `ارتقا ${upgradeType}`, description: `افزایش ${priceDiff.toLocaleString('fa-IR')} تومان`, priceDiff };
    }

    return { status: 'unavailable', buttonText: 'در دسترس نیست', description: 'مدت زمان کمتر از اشتراک فعلی شماست' };
  };
  ...
  const handlePurchase = async (planId: 'bronze' | 'silver' | 'gold') => {
    ...
    try {
      setLoading(true);
      const res = await userService.createSubscription({ planId, duration: selectedDuration });
  ...
  const getDiscount = (monthly: number, other: number, months: number) => {
    ...
    const discount = Math.round(((monthly - equivalentMonthly) / monthly) * 100);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12">
          {currentSubscription ? (
            <div className="bg-gradient-to-r from-green-50 to-emerald-100 border border-green-200 rounded-xl p-6 text-center shadow-sm">
              <div className="flex items-center justify-center gap-3 mb-3">
                <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
                <h2 className="text-2xl font-bold text-green-800">اشتراک فعلی شما</h2>
              </div>
              <div className="text-lg font-bold text-gray-800 mb-2">
                پلن {getPlanName(currentSubscription.plan)} - {DURATION_LABELS[currentSubscription.duration]}
              </div>
              <div className="text-gray-700 space-x-1 space-x-reverse">
                <span>منقضی می‌شود:</span>
                <span className="font-medium">{new Date(currentSubscription.expiresAt).toLocaleDateString('fa-IR')}</span>
              </div>
              <div className="mt-3 flex items-center justify-center gap-4">
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow">
                  <span className="text-sm text-gray-600">مبلغ پرداختی:</span>
                  <span className="font-bold text-red-600">{currentSubscription.amount.toLocaleString('fa-IR')} تومان</span>
                </div>
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow">
                  <span className="text-sm text-gray-600">مدت:</span>
                  <span className="font-medium capitalize">{DURATION_LABELS[currentSubscription.duration]}</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="bg-gradient-to-r from-yellow-50 to-amber-100 border border-yellow-200 rounded-xl p-6 text-center shadow-sm">
              <div className="flex items-center justify-center gap-3 mb-3">
                <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                <h2 className="text-2xl font-bold text-yellow-800">بدون اشتراک فعال</h2>
              </div>
              <p className="text-gray-700 text-lg">هنوز اشتراکی خریداری نکرده‌اید. پلن مناسب خود را انتخاب کنید!</p>
            </div>
          )}
        </div>

        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-800 mb-4">پلن‌های اشتراک</h1>
          ...
          {plans.map((plan) => {
            ...
            return (
              ...
              >
                ...
                {/* Pricing */}
                <div className="text-center mb-6">
                  <div className={`text-3xl font-bold mb-2 ${planStatus.status === 'upgrade' ? 'text-blue-600' : 'text-red-600'}`}>
                  ...
                  {!isMonthly && (
                    <div className="text-sm text-gray-500">
                      {isQuarterly && `(~${formatPrice(Math.round(plan.price.quarterly / 3))} در ماه)`}
                      {isYearly && `(~${formatPrice(Math.round(plan.price.yearly / 12))} در ماه)`}
                    </div>
                  )}
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start">
                      <span className={`mr-2 mt-1 ${planStatus.status === 'active' ? 'text-green-500' : planStatus.status === 'upgrade' ? 'text-blue-500' : 'text-green-500'}`}>✓</span>
        ...
        <div className="mt-16 text-center text-gray-600 max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-sm">
          <p className="mb-2 font-medium">💡 هرچه مدت بیشتری خریداری کنید، تخفیف بیشتری دریافت می‌کنید!</p>
          <p className="text-sm">پرداخت امن از طریق درگاه‌های معتبر بانکی • لغو اشتراک تا ۲۴ ساعت قبل از انقضا</p>
        </div>
      </div>
    </div>
  );
